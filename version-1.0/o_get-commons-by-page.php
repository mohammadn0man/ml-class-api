...
if ($db && $S && $Data) {
    ...
    switch ($Type) {
        ...
        case 4:
            ...
            array_push($arr, $Token);
            $Row1 = $db->select("SELECT token(`question`.`question_id`,'question') AS token,`question`.`type_id`, `question`.`body`, IFNULL(CONCAT('web-photo/question/',`question`.`image`),'') AS `image`, `question`.`option1`,  IFNULL(`question`.`option2`,'') AS `option2`, IFNULL(`question`.`option3`,'') AS `option3`, IFNULL(`question`.`option4`,'') AS `option4`, IFNULL(`question`.`option5`,'') AS `option5`, IFNULL(`question`.`option6`,'') AS `option6`, `question`.`selected`,CONCAT('Add By : ',`user`.`name`,' on ',DATE_FORMAT(`question`.`add_date`,'%e %b, %Y %h:%i:%s %p')) AS add_date,`question`.`status`,`question`.`add_by`=" . $S["user_id"] . "  AS is_delete FROM `question` INNER JOIN `user` ON `user`.`user_id`=`question`.`add_by` WHERE `question`.`type_id`=? AND token(`question`.`chapter_id`,'chapter')=? $DataBySearch LIMIT $Start,10", $arr);
            break;
        case 5:
            $Row1 = $db->select("SELECT token(`user`.`user_id`,'user') AS token,`user`.`name`,`user`.`email`,`user`.`mobile`,CONCAT('Add By : ',IFNULL(`u1`.`name`,'N/A'),' on ',DATE_FORMAT(`user`.`add_date`,'%e %b, %Y %h:%i:%s %p')) AS add_date,`user`.`status`,CASE WHEN `user`.`type`=2 THEN 'user' WHEN `user`.`type`=1 THEN 'Admin' else 'SubAdmin' end as type FROM `user` left join `user` AS `u1` ON `u1`.`user_id`=`user`.`add_by` $DataBySearch LIMIT $Start,10", $arr);
            break;
        case 6:
            $CPTR = $db->selectRow("SELECT `chapter`.`chapter_id` FROM `chapter` WHERE token(`chapter`.`chapter_id`,'chapter')=?", array($Token));
            $Row1 = $db->select("SELECT `type`.`type_id` AS token, `type`.`name`,CONCAT('Total Question : ',question_count(?,`type`.`type_id`)) AS total,CONCAT('Add By : ',`user`.`name`,' on ',DATE_FORMAT(`type`.`add_date`,'%e %b, %Y %h:%i:%s %p')) AS add_date,1 AS `status`,0 AS is_delete FROM `type` INNER JOIN `user` ON `user`.`user_id`=`type`.`add_by` LIMIT $Start,10", array($CPTR["chapter_id"]));
            break;
        default:
            throw new Exception("Security error.");
    }
